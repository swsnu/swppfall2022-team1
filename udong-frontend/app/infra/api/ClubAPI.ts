...
export const ClubAPI = (() => {
    ...
    async function leaveClub(clubId: number): Promise<void> {
        return await axiosConfig.delete(`/api/club/${clubId}/user/`)
    }
    ...
    async function removeClubMember(clubId: number, userId: number): Promise<void> {
        return await axiosConfig.delete(`/api/club/${clubId}/user/${userId}/`)
    }
    ...
    async function createClubPost(clubId: number, post: CreatePost): Promise<BoardPost> {
        const postDto = createPostTransformer.toDto(post)
